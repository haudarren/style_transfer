...
def _conv2d_relu(vgg_layers, prev_layer, layer, layer_name):
    ...
    with tf.variable_scope(layer_name) as scope:
    	...
    	# weights = tf.get_variable('weights', initializer=tf.constant(W))
    	# biases = tf.get_variable('biases', initializer=tf.constant(b))
    	conv = tf.nn.conv2d(prev_layer, weights, [1, 1, 1, 1], padding='SAME')
    	conv = tf.nn.relu(conv + biases, name=scope.name)
...
def _avgpool(prev_layer):
    ...
    Hint for choosing strides and ksize: choose what you feel appropriate
    """
