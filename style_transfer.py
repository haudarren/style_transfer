...
def _create_content_loss(p, f):
    ...
    """
    s = tf.to_float(tf.reduce_prod(p.shape))
    content_loss = 1/(4*s)*tf.square(tf.norm(f - p))
    return content_loss
...
def _gram_matrix(F, N, M):
    ...
    return tf.matmul(F_2d, F_2d, transpose_a=True)
...
def _single_style_loss(a, g):
    ...
    """
    feature_map_dims = a.shape
    N = feature_map_dims[3]
    M = feature_map_dims[1]*feature_map_dims[2]

    # Get Gram matrices of feature representations of the real and generated images.
    A = _gram_matrix(a, N, M)
    G = _gram_matrix(g, N, M)

    # Calculate layer style loss.
    E_l = 1/(4*tf.square(float(N))*tf.square(float(M))) * tf.square(tf.norm(G - A))
    return E_l
    ...
    with tf.Session() as sess:
        ...
        for index in range(initial_step, ITERS):
            ...
            if (index + 1) % skip_step == 0:
                ...
                filename = os.path.join(outputs_dir, '%d.png' % (index))
                ...
                if (index + 1) % 20 == 0:
                    ...
                    saver.save(sess, os.path.join(checkpoints_dir, 'style_transfer'), index)
